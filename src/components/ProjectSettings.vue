    ...
    <div v-if="editValue" id="custom" class="absolute z-20 overflow-hidden bg-black bg-opacity-30 left-0 right-0 top-0 h-screen w-full">
        <div class="h-screen w-full">
            <div class="absolute inset-0 mx-auto my-auto w-full max-w-screen-md h-min bg-slate-400 p-4 rounded-md">
                <div v-if="projects.showPermissions == false" class="grid grid-cols-12 gap-4">
                    <div class="col-start-1 col-span-11 text-slate-200 poppins mx-2 text-xl">
                        Settings
                    </div>
                    <button class="col-start-12 col-span-1 dark:bg-slate-500 hover:dark:bg-yellow-400 hover:text-slate-800 text-slate-200 poppins z-30 opacity-100 rounded-md p-2" @click="toggleEdit()"> X</button>
                    ...
                    <div class="col-span-12 poppins dark:text-slate-200 text-2xl">Project name</div>
                        <div v-if="editName" class="col-span-12 open-sans text-xl dark:text-slate-200">
                            <form class="grid grid-cols-12 gap-4" @submit.prevent="changeName">
                                ...
                                <!-- SAVE BUTTON -->
                                <EditButton v-if="editName" color="dark:bg-green-400" text="Save" position="col-start-9"/>

                                <!-- CANCEL BUTTON -->
                                <EditButton v-if="editName" @click="editName = !editName" text="Cancel"/>
                            </form>
                        </div>
                    ...
                    <!-- EDIT BUTTON-->
                    <EditButton v-if="!editName" @click="editInputToggle()" text="Edit"/>
                    ...
                    <div class="col-span-12 poppins dark:text-slate-200 text-2xl">Add User</div>
                        <div class="col-span-12 open-sans text-xl dark:text-slate-200">
                            <form class="grid grid-cols-12 gap-4" @submit.prevent="addUser">
                                ...
                                <div v-if="projects.addUserError" class="text-red-500 poppins col-span-8 px-6">
                                {{ projects.addUserError }}
                                </div>

                                <input data-focus="searchUser" @input="searchUser" type="text" :class="{'border border-red-500': projects.addUserError}" class="py-2 px-4 mx-2 col-span-8 outline-none dark:bg-slate-500 rounded-md w-full" v-model="search">
                    ...
                    <div class="col-span-12 overflow-hidden">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-8" v-for="user in projects.currentProjectUsers" :key="user.permissions_name">
                        ...
                        <!-- PERMISSIONS -->
                        <div class="col-span-8 poppins dark:text-slate-200 text-2xl max-h-96 p-2">Permissions</div>
                            <EditButton text="Edit" @click="showPermissions"></EditButton>
                        ...
                        <div class="col-span-12 py-2">
                            <div class="grid grid-cols-12 gap-4">
                                ...
                                <div class="col-span-8 py-2 my-auto">
                                    ...
                                    <div class="text-red-500 text-xl poppins" v-if="projects.leaveErrors">
                                    {{ projects.leaveErrors }}
                                    </div>
                                </div>
                                <EditButton @click="leaveProject()" text="Leave" color="bg-red-700" additional="max-h-12"/>
                            </div>
                        </div>
                </div>
