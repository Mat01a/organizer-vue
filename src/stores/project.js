...
export const useProjectStore = defineStore('project', {
	...
	actions: {
		...
		async searchUser(id, name) {
			this.loading = true;
			await instance
				.get('api/projects/' + id + '/users/' + name)
				.then((response) => {
			...
			await instance
				.get(
					'api/projects/' +
						id +
						'/users?page=' +
						(this.pagesOfUsers.current_page + 1)
				)
				.then((response) => {
					this.currentProjectUsers = response.data.data;
		...
		async changeProjectStatus(id, status) {
			...
			/**
			 * Generalnie jak używasz awaita to nie musisz "thenować"
			 */
			const response = await instance
				.post('/api/projects/change-status', {
					project_id: id,
					status: new_status,
				})
				.catch((error) => {
		...
		async leaveProject(id) {
			...
			await instance
				...
				/**
				 * A co jeśli response status jest większy niż 400?
				 */
				.then((response) => {
		...
		async getCurrentProjectPermissions(id) {
			this.loading = true;
			/**
			 * Axios ma fajne API do wprowadzania parametrów
			 */
			await instance
		...
		async addNewPermissin(id, name) {
			this.loading = true;
			await instance
				.post('/api/projects/add-permission', {
					project_id: id,
					name: name,
				})
				.then((response) => {
			...
			await instance
				.get(
					'api/projects/' +
						id +
						'/permissions?page=' +
						(this.currentProjectPermissions.current_page + 1)
				)
				.then((response) => {
					this.currentProjectPermissions = response.data;
					this.loading = false;
		...
		async changeUserPermission(project_id, user, permission_id) {
			...
			this.loading = true;
			await instance
				.patch('/api/projects/update-user-permission', {
		...
		async changePermission(permission_id, form, project_id) {
			...
			this.hideError();
			await instance
				.patch('api/projects/update-permission-settings', {
					project_id: project_id,
					permission_id: permission_id,
					name: form.value.name,
		...
		async removePermission(project_id, permission_id) {
			this.loading = true;
			await instance
				.patch('api/projects/remove-permission', {
					project_id: project_id,
					permission_id: permission_id,
				})
				.then((response) => {
