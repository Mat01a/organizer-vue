...
export const useUserStore = defineStore('user', {
	...
	actions: {
		...
		async login(form) {
			this.loading = true;
			await instance.get('/sanctum/csrf-cookie').catch((error) => {
				this.loading = false;
			});
		...
		async register(form) {
			...
			/**
			 * Dużo niepotrzebnych deklaracji zmiennych.
			 *
			 * "register" nie jest używane
			 * "respponse" nie jest używane
			 *
			 * tip: możesz sobie skonfigurować eslinta aby sprawdzał ci/usuwał niepotrzebne zmienne i importy
			 */
			const register = await instance
				...
				.catch((error) => {
					...
					for (let item in error.response.data.errors) {
						this.register_message_error[item] =
